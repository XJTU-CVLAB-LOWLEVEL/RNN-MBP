...
class DeblurDataset(Dataset):
    ...
    def __init__(self, path, frames, future_frames, past_frames, crop_size=(256, 256),
    ...
    def _generate_samples(self, dataset_path):
        ...
        for seq in seqs:
            records[seq] = list()
            seq_path = join(dataset_path, seq, 'sharp')
            imgs =  [x for x in sorted(os.listdir(seq_path))]
            for img in imgs:
                suffix = 'png'
                sample = dict()
                sample['Blur'] = join(dataset_path, seq, 'blur_gamma', img)
                sample['Sharp'] = join(dataset_path, seq, 'sharp', img)
...
class Dataloader:
    def __init__(self, para, device_id, ds_type='train'):
        path = join(para.data_root, 'GOPRO_Large', 'train')
        frames = para.frames
